...
namespace LeattyServer.ServerInfo.Inventory
{
    public class MapleInventory
    {
        ...
        {
            if (amount > 0 && Mesos == GameConstants.MAX_MESOS)
                return;

            if (Mesos + amount < 0)
                Mesos = 0;
            else if (Mesos + amount > GameConstants.MAX_MESOS)
                Mesos = GameConstants.MAX_MESOS;
            else
            {
                Mesos += amount;
            }
        ...
        private Dictionary<short, MapleItem> GetInventory(MapleInventoryType inventoryType)
        {
            ...
            foreach (InventoryItem dbInventoryItem in dbInventoryItems)
            {
                ...
                if (type == MapleInventoryType.Equip)
                {
                    ...
                    if (dbInventoryEquip != null)
                    {
                        ...
                        equip.IncMhp = dbInventoryEquip.IncMhp;
                        equip.IncMmp = dbInventoryEquip.IncMmp;
        ...
        public void SaveToDatabase(bool insert = false)
        {
            ...
            using (LeattyContext dbContext = new LeattyContext())
            {
                ...
                if (updateSlot != null && !insert)
                {
                    ...
                    dbContext.Entry(updateSlot).State = System.Data.Entity.EntityState.Modified;
                }
                else
                {
        ...
        public static void UpdateMesos(MapleClient c, long mesos)
        {
        ...
        public static class Packets
        {
            ...
            public static PacketWriter ShowOperations(List<InventoryOperation> operations)
            {
                ...
                pw.WriteHeader(SendHeader.InventoryOperation);
            ...
            public static void AddInventoryInfo(PacketWriter pw, MapleInventory inventory)
            {
                ...
                pw.WriteInt(0);
                pw.WriteInt(0); //new v137
                pw.WriteInt(0); //new V135
            ...
            {
                PacketWriter pw = new PacketWriter();
                pw.WriteHeader(SendHeader.UpdateCharLook);
                pw.WriteInt(chr.Id);
                pw.WriteByte(1);
