...
namespace LeattyServer.Helpers
{
    class ParseWzFormula
    {
        ...
        public static int ParseInt(string formula, int x, int skillId = 0)
        {
            //try if it's a static
            int result;
            if (int.TryParse(formula, out result))
                return result;
            
            //else the formula must be parsed
            //ex: 10+2*d(x/5)
            MathParser parser = new MathParser();
            parser.LocalVariables.Add("x", x);
            parser.LocalFunctions.Add("d", delegate(decimal[] d)
            {
                return Math.Floor(d[0]);
            });
            parser.LocalFunctions.Add("u", delegate(decimal[] d)
            {
                return Math.Ceiling(d[0]);
            });


            try
            {
                result = Convert.ToInt32(parser.Parse(formula.StartsWith("-") ? ("0"+ formula) : formula));
                return result;
            }
            catch (Exception ex)
            {
                ServerConsole.Warning(String.Format("Error trying to parse {0} with x = {1}, skill = {2}", formula, x, skillId));
