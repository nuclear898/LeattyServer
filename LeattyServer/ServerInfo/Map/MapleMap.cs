...
namespace LeattyServer.ServerInfo.Map
{
    public class MapleMap
    {
        ...
        public void ShowCharacters(MapleCharacter chr)
        {
            lock (Characters)
            {
                foreach (var characterKVP in Characters.Where(x => x.Key != chr.Id))
                {
                    ...
                    chr.Client.SendPacket(MoveCharacterHandler.CharacterMovePacket(chr.Id, new List<Movement.MapleMovementFragment>(), chr.Position));
                }
            }
        }
        ...
        public void EnterPortal(MapleClient c, string portalName)
        {
            ...
            if (portal != null)
            {
                if (Functions.Distance(portal.Position, c.Account.Character.Position) >= 50)
                    c.CheatTracker.AddOffence(OffenceType.AbnormalValues);

                MapleMap toMap = Program.GetChannelServer(c.Channel).GetMap(portal.ToMap);
                if (toMap != null)
                {
        ...
        public void EnterPortalSpecial(MapleClient c, string portalName)
        {
            ...
            {
                ServerConsole.Error("Unable to enter portal " + portalName + " in map id " + c.Account.Character.MapId);
                c.Account.Character.SendBlueMessage(portalName + " not scripted yet " + c.Account.Character.MapId);
            }
        }
        #endregion
        ...
        public Point? CheckMobPositions(OffenceTracker OT)
        {
            Point? FirstPos = null;
            if (Mobs.Count > 3)
            {
                Point? PrevPos = null;
                Dictionary<Int32, Double> TotalDistances = new Dictionary<Int32, Double>();
                foreach (MapleMonster Mob in Mobs.Values)
                {
                    if (!TotalDistances.ContainsKey(Mob.WzInfo.MobId))
                        TotalDistances.Add(Mob.WzInfo.MobId, 0);

                    if (PrevPos != null && FirstPos != null)
                    {
                        TotalDistances[Mob.WzInfo.MobId] += Functions.Distance(FirstPos, Mob.Position);
                        TotalDistances[Mob.WzInfo.MobId] += Functions.Distance(PrevPos, Mob.Position);
                    }

                    //The value is already to high, break
                    if (TotalDistances[Mob.WzInfo.MobId] > Mobs.Count * 20)
                        break;

                    if (FirstPos == null)
                        FirstPos = Mob.Position;
                    PrevPos = Mob.Position;
                }

                Double TotalDistance = 0;
                foreach (Double Distance in TotalDistances.Values)
                    TotalDistance += Distance;

                //Thus if all mobs have a min distance to each other of 5px, add offence
                if (TotalDistance < (Mobs.Count * 5))
                {
                    OT.AddOffence(OffenceType.PosibleVac);
                }
            }
            return FirstPos;
        }
        ...
        public void SpawnMapItemsFromMonster(MapleMonster mob, Point sourcePosition, MapleCharacter dropOwner)
        {
            ...
            {
                int chance = (int)(mobDrop.DropChance * (dropOwner.Stats.DropR / 100.0) * ServerConstants.DropRate);
                if (Functions.Random(0, 999999) <= chance)
                {
                    ...
                    if (mobDrop.ItemId == 0) //meso
                    {
                        ...
                        if (amount > 0)
                        {
                            amount = (int)(amount * (dropOwner.Stats.MesoR / 100.0) * ServerConstants.MesoRate);
        ...
        public bool HandlePlayerItemPickup(MapleClient c, int mapItemObjectId)
        {
            lock (MapItems)
            {
                ...
                if (mapItem.PlayerDrop || mapItem.DropType == MapleDropType.FreeForAll || mapItem.DropType == MapleDropType.Boss || mapItem.DropType == MapleDropType.Unk || (mapItem.DropType == 0 && mapItem.OwnerId == c.Account.Character.Id))  //TODO: party, dropType == 1 and ownerId is in player's party
                {
                    ...
                    if (mapItem.Meso > 0)
                    {
                        if (chr.Inventory.Mesos < GameConstants.MAX_MESOS)
                        {
        ...
        public static PacketWriter SpawnReactor(int objectId, WzMap.Reactor Reactor)
        {
            ...
            pw.WriteHeader(SendHeader.SpawnReactor);
        ...
        {
            PacketWriter pw = new PacketWriter();
            pw.WriteHeader(SendHeader.SpawnNpc);
            pw.WriteInt(objectId);
            pw.WriteInt(Npc.Id);
            pw.WriteShort(Npc.x);
            ...
            pw.WriteInt(0);//unk v143
            //v158:
            pw.WriteByte(0);
            pw.WriteInt(-1);
            pw.WriteZeroBytes(11);
            return pw;
        }
        #endregion
